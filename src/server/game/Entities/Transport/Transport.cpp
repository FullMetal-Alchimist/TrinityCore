...
void Transport::UpdatePlayerPositions()
{
    for (PlayerSet::iterator itr = m_passengers.begin() ; itr != m_passengers.end() ; ++itr)
    {
        ...
        x = GetPositionX() + (plr->m_movementInfo.transport.pos.m_positionX * cos(GetOrientation()) + plr->m_movementInfo.transport.pos.m_positionY * sin(GetOrientation() + float(M_PI)));
